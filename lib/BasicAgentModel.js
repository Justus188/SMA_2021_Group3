...
const probArrival = 0.5;//03371734; // mean interarrival measured to be 29.15, pexp(1, 29.15) = prob Arrival per s
const serviceTime = 500;//2704; // mean servicetime measured to be 2704s
...
function updateCustomer(customerId){
	...
	switch(state){
		...
		case SHOPPING: //Rae
			...
			if (empty_staging_spot == 0 && empty_table !== undefined){
				//update state
		...
		case STAGING: // Moktar
			...
			if (empty_table !== undefined){	
				...
				stats[0].cumulativeValue = customer.timeEnter - customer.timeQR;
				stats[0].count++;
			} else if(hasArrived && STAGING_SPOT[customerposition-1]==0){
				//update state
				STAGING_SPOT[customerposition]=0;
				STAGING_SPOT[customerposition-1]=1;
				//update target
				customer.target.col=col-1;
			}
		break;
